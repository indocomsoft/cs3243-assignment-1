...
It is given in the lecture notes that the time complexity of A* Search is $O(b^{h^*(s_0)-h(s_0)})$. Since $h_2(s) \geq h_1(s)$ for all states $s$ due to Linear Conflict being an extension of Manhattan Distance, $(h^*(s_0)-h_2(s_0)) \leq (h^*(s_0)-h_1(s_0))$ and A* Search with Linear Conflict has a lower time complexity than with Manhattan Distance. Based on the results, the number of nodes generated by $h_2$ is about $0.5-0.67$ times that generated by $h_1$. We shall take the worst case, which is $0.67$, or $2/3$. With branching factor $b = 4$ when the empty space is in the center, we have $4^{h^*(s_0)-h_2(s_0)} = 0.67*4^{h^*(s_0)-h_1(s_0)}$. \textbf{Julius pls figure out where to take this equation or maybe delete it}

By reaching the goal node faster and generating less nodes along the way, the memory consumption, denoted by max frontier size, for $h_2$ is less than $h_1$. The relative proportions of memory consumption is similar to the proportions of number of nodes generated for the same solvable test case.

For the unsolvable test case, $h_2$ seems to consume more memory than $h_1$. Results were similar for other unsolvable test cases of our own construction. We then ran the same unsolvable test cases through a third heuristic $h_3$, which is the Misplaced Tiles heuristic from the lectures. $h_3$ showed higher memory consumption than both $h_1$ and $h_2$. So in terms of dominance, we have $h_2 \geq h_1 \geq h_3$, but for memory consumption we have $h_3 > h_2 > h_1$. We thus conclude that the max frontier size is not (solely) affected by dominance, and this inverted memory consumption behaviour for unsolvable test cases is beyond our ability to reason out.
